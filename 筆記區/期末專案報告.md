...
```mermaid
graph TB
    subgraph APP["🎮 Habit Hero App"]
        subgraph UI["📱 UI Layer - 介面層"]
            Home["🏠 HomeScreen<br/>首頁"]
            Quest["⚔️ QuestScreen<br/>任務頁面"]
            Achievement["🏆 AchievementScreen<br/>成就頁面"]
            Stats["📊 StatsScreen<br/>統計頁面"]
            Settings["⚙️ SettingsScreen<br/>設定頁面"]
        ...
        subgraph State["🔄 State Management Layer - 狀態管理層"]
            PlayerProvider["PlayerProvider<br/>玩家狀態"]
            QuestProvider["QuestProvider<br/>任務狀態"]
        end

        subgraph Logic["⚙️ Business Logic Layer - 業務邏輯層"]
            QuestService["QuestService<br/>任務邏輯"]
            PlayerService["PlayerService<br/>玩家邏輯"]
            AchievementService["AchievementService<br/>成就邏輯"]
        end

        subgraph Model["📦 Model Layer - 資料模型層"]
            QuestModel["Quest<br/>«abstract»"]
            PlayerModel["Player"]
        ...
        subgraph Storage["💾 Storage Layer - 資料儲存層"]
            SharedPrefs["SharedPreferences<br/>本地鍵值儲存"]
        end
    end
...
flowchart TD
    ...
    subgraph Main["📱 主要介面"]
        Home["🏠 首頁 Home<br/>• 角色狀態<br/>• 經驗值進度<br/>• 今日任務摘要"]
    ...
    subgraph Pages["📑 頁面導航"]
        Nav -->|🏠 首頁| Home
        Nav -->|⚔️ 任務| QuestPage
        Nav -->|🏆 成就| AchievementPage
        Nav -->|📊 統計| StatsPage
        Nav -->|⚙️ 設定| SettingsPage
    end

    subgraph QuestFlow["⚔️ 任務完成流程"]
        QuestPage["📋 任務頁面<br/>• 每日任務<br/>• 每週任務"]
        ClickQuest["👆 點擊任務"]
        MarkComplete{"✅ 標記完成？"}
        GetExp["✨ 獲得經驗值<br/>+EXP 動畫"]
        CheckLevelUp{"📈 檢查升級？"}
        LevelUp["🎉 升級動畫<br/>Level Up!"]
        CheckAchievement{"🏅 檢查成就？"}
        UnlockAchievement["🎖️ 解鎖成就"]

        QuestPage --> ClickQuest
        ClickQuest --> MarkComplete
        MarkComplete -->|是| GetExp
        MarkComplete -->|否| QuestPage
        GetExp --> CheckLevelUp
        CheckLevelUp -->|是| LevelUp
        CheckLevelUp -->|否| Home
        LevelUp --> CheckAchievement
        CheckAchievement -->|有新成就| UnlockAchievement
        CheckAchievement -->|無| Home
        UnlockAchievement --> Home
    end

    subgraph OtherPages["📄 其他頁面"]
        AchievementPage["🏆 成就頁面<br/>• 成就徽章<br/>• 解鎖進度"]
        StatsPage["📊 統計頁面<br/>• 完成率<br/>• 趨勢圖表"]
        SettingsPage["⚙️ 設定頁面<br/>• 語言切換<br/>• 主題設定"]
    end
...
| 首頁 | 任務 | 成就 | 統計 | 設定 |
|:----:|:----:|:----:|:----:|:----:|
| ![首頁](screenshots/home.png) | ![任務](screenshots/quest.png) | ![成就](screenshots/achievement.png) | ![統計](screenshots/stats.png) | ![設定](screenshots/settings.png) |
...
```
https://github.com/___________/app_end
```
